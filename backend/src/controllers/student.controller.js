...
export const getStudentMarks = async (req, res) => {
  try {
    ...
    for (const mapping of section.courseFacultyMappings) {
      ...
      if (marksRecords.length > 0) {
        ...
        const averagePercentage =
          totalPercentageForCourse / marksRecords.length;
    ...
    const currentSemesterAverage =
      courseCount > 0 ? totalPercentage / courseCount : 0;

    // Prepare response data
    const marksData = {
      overall: {
        totalCredits,
        currentSemesterAverage: Number.parseFloat(
...
export const getCourseMarks = async (req, res) => {
  try {
    ...
    for (const mapping of section.courseFacultyMappings) {
      ...
      const examTypes = [
        ...new Set(marksRecords.map((record) => record.examType)),
      ];
      ...
      let totalMarksObtained = assessments.reduce(
        (sum, assessment) => sum + assessment.marksScored,
        0,
      );
      let totalMaxMarks = assessments.reduce(
        (sum, assessment) => sum + assessment.maxMarks,
        0,
      );
    ...
    for (const mapping of section.courseFacultyMappings) {
      ...
      const sectionAverage =
        sectionStudentCount > 0
          ? sectionTotalPercentage / sectionStudentCount
          : 0;
      ...
      if (comparisonData.length === 0 && studentMarks.length > 0) {
        ...
        const examTypes = [
          ...new Set(studentMarks.map((mark) => mark.examType)),
        ];
        ...
        for (const examType of examTypes) {
          ...
          for (const sectionStudent of studentsInSection) {
            ...
            if (sectionStudentExamMark) {
              sectionExamTotal +=
                (sectionStudentExamMark.marksScored /
                  sectionStudentExamMark.totalMarks) *
                100;
              sectionExamCount++;
            }
          }

          const sectionExamAverage =
            sectionExamCount > 0 ? sectionExamTotal / sectionExamCount : 0;

          trendData.push({
            examName: examType.replace(/-/g, " "),
            yourScore: studentExamMark
              ? (studentExamMark.marksScored / studentExamMark.totalMarks) * 100
              : 0,
