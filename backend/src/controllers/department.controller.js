...
export const addDepartment = async (req, res) => {
  try {
    const { name, description, location, departmentCode, dateEstablished } =
      req.body;
    ...
    if (existingDepartment) {
      return res.status(409).json({
        message:
          "A department with this code already exists in the institution",
        data: [],
        status: 409,
      });
    }
...
export const updateDepartment = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, departmentCode, description, location, dateEstablished } =
      req.body;
