...
export const bulkAddMarks = async (req, res) => {
  try {
    ...
    if (!sectionId || !courseId || !facultyId || !examType) {
      const errorMessage =
        "Section ID, Course ID, Faculty ID, and Exam Type are required";
    ...
    if (existingMarks) {
      const errorMessage =
        "Marks have already been added for this section, course, faculty, and exam type";
    ...
    const bulkOps = marksData.map(({ studentId, marksScored }) => {
      ...
      if (marksScored < 0 || marksScored > totalMarks) {
        throw new Error(
          `Marks scored for student ${studentId} must be between 0 and ${totalMarks}`,
        );
      }
...
export const bulkUpdateMarks = async (req, res) => {
  try {
    ...
    if (existingMarks.length === 0) {
      const errorMessage =
        "No marks found for this section, course, faculty, and exam type";
