...
export const bulkMarkAttendance = async (req, res) => {
  try {
    ...
    if (existingAttendance) {
      const errorMessage =
        "Attendance has already been marked for this section, course, and faculty today";
    ...
    const bulkOps = attendanceData.map(
      ({ studentId, status, date: recordDate }) => {
        const recordFormattedDate = new Date(
          (recordDate ? new Date(recordDate) : formattedDate)
            .toISOString()
            .split("T")[0],
...
export const isAttendanceMarked = async (req, res) => {
  try {
    ...
    if (!section) {
      return res
        .status(404)
        .json({ message: "Section not found", data: [], code: 404 });
    }
    ...
    if (!semester) {
      return res
        .status(404)
        .json({ message: "Semester not found", data: [], code: 404 });
    }
    ...
    const formattedDates = markedDates.map(
