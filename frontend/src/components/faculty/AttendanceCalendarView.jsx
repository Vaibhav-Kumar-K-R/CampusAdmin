...
export function AttendanceCalendarView({
  ...
  const isSpecialDate = (date) =>
    specialDates.some((specialDate) => isSameDay(date, parseISO(specialDate)));

  const hasAttendance = (date) =>
    attendanceDatesMap.has(format(date, "yyyy-MM-dd"));
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {daysInMonth.map((day) => {
            ...
            let dayClasses =
              "h-10 flex items-center justify-center text-sm rounded-full relative";
            ...
            if (isDisabled) {
              ...
              if (isSelected) {
                dayClasses += " bg-[#048c7f] text-white";
              } else if (isMarked) {
        ...
        <div className="mt-6 text-sm">
          ...
          <p className="mt-4 text-muted-foreground">
            Total days with attendance: {attendanceHistory.length}
          </p>
        </div>
      </CardContent>
    </Card>
