    ...
    const fetchStudents = async () => {
      try {
            ...
            for (const studentId of section.students) {
              ...
              if (courseMapping) {
                ...
                const courseResponse = await axios.get(
                  `${apiUrl}/api/course/${courseMapping.courseId._id}`,
                  config,
                );
        ...
        const studentPromises = Array.from(studentIds).map((studentId) =>
          axios.get(`${apiUrl}/api/student/${studentId._id}`, config),
        );

        const studentResponses = await Promise.all(studentPromises);

        const studentData = studentResponses.map(
          (response) => response.data.data.student,
        );
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500">
                            {studentCourses[student._id] ? (
                              <div className="space-y-1">
                                {studentCourses[student._id].map(
                                  (course, index) => (
                                    <div
                                      key={index}
