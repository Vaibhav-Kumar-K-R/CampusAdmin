...
const MarksTab = ({ studentData }) => {
  ...
  courses.forEach((course) => {
    course.assessments.forEach((assessment) => {
      ...
      transformedData[assessment.name][course.name] =
        (assessment.marksScored / assessment.maxMarks) * 100;
    });
  });
  ...
  const hasNoAssessments = courses.every(
        ...
        <motion.div variants={itemVariants} className="md:col-span-2">
          <Card className="border-none shadow-md hover:shadow-lg transition-shadow h-full">
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                Your current semester academic performance
              </CardDescription>
            </CardHeader>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <div className="text-3xl font-bold">
                    {Number(marksData?.currentSemesterAverage || 0).toFixed(1)}%
                  </div>
            ...
            <Card className="border-none shadow-md hover:shadow-lg transition-shadow h-full">
              ...
              <CardContent className="pt-0">
                <div className="flex items-center justify-between">
                  {/* Fix: Ensure percentage is a valid number between 0-100 */}
                  <div className="text-3xl font-bold">
                    {Math.min(course.totalWeightedPercentage || 0, 100)}%
                  </div>
                  <Badge
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Grade Points: {course.gradePoints.toFixed(1)}
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <CardContent>
            {hasNoAssessments ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm">
                  Assessment data will appear here once your instructors have
                  recorded your marks.
                </p>
              </div>
            ) : (
              ...
              >
                <TabsList
                  className="mb-4 flex flex-nowrap overflow-x-auto pb-1"
                          ...
                          <div className="space-y-4">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-bold">
                                {Math.min(
                                  course.totalWeightedPercentage || 0,
                                  100,
                                )}
                                %
                              </span>
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <Badge
                                className="text-white"
                                style={{
                                  backgroundColor: GRADE_COLORS[course.grade],
                                }}
                              >
                                {course.grade}
                              </Badge>
      ...
      <motion.div variants={itemVariants}>
        <Card className="border-none shadow-md hover:shadow-lg transition-shadow">
          <CardHeader>
            ...
            <CardDescription>
              Your performance trend throughout the semester
            </CardDescription>
          </CardHeader>
          <CardContent>
            {semesterProgress.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm">
                  Progress data will appear here once assessments have been
                  recorded.
                </p>
              </div>
            ) : (
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis dataKey="name" />
                    <YAxis domain={[0, 100]} />{" "}
                    {/* Y-axis now represents percentages */}
                    <Tooltip />
                    <Legend />
                    {courses.map((course, index) => (
                      <Line
      ...
      <motion.div variants={itemVariants}>
        <Card className="border-none shadow-md hover:shadow-lg transition-shadow">
          <CardHeader>
            ...
            <CardDescription>
              Your grades across all courses this semester
            </CardDescription>
          </CardHeader>
          <CardContent>
            {courses.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm">
                  Grade distribution will appear here once courses have been
                  graded.
                </p>
              </div>
            ) : (
