...
const SectionsTab = () => {
  ...
  const [isCourseFacultyDialogOpen, setIsCourseFacultyDialogOpen] =
    useState(false);
  ...
  const [isViewMappingsDialogOpen, setIsViewMappingsDialogOpen] =
    useState(false);
  const [selectedSectionMappings, setSelectedSectionMappings] = useState([]);
  const [selectedSectionName, setSelectedSectionName] = useState("");
  const [isDeleteMappingDialogOpen, setIsDeleteMappingDialogOpen] =
    useState(false);
    ...
    try {
      ...
      if (filterSemester !== "all") {
        filteredSections = filteredSections.filter(
          (section) =>
            section.semesterId && section.semesterId._id === filterSemester,
        );
      }
      ...
      if (searchQuery) {
        ...
        filteredSections = filteredSections.filter(
          (section) =>
            ...
            (section.semesterId &&
              section.semesterId.name.toLowerCase().includes(query)),
        );
      }
  ...
  const fetchCourses = async (departmentId) => {
    try {
      ...
      const filteredCourses = departmentId
        ? allCourses.filter(
            (course) => course.departmentId._id === departmentId,
          )
        : allCourses;
  ...
  const fetchFaculties = async (departmentId) => {
    try {
      ...
      const filteredFaculties = departmentId
        ? allFaculties.filter(
            (faculty) => faculty.departmentId._id === departmentId,
          )
        : allFaculties;
  ...
  const handleAddSection = async () => {
    ...
    try {
      ...
      if (response.data.code === 201) {
        ...
        setServerError(
          response.data.message || "Failed to add section. Please try again.",
        );
      }
    } catch (err) {
      ...
      const errorMessage =
        ...
        "Failed to add section. Please check your connection and try again.";
  ...
  const handleUpdateSection = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/api/section/${currentSection._id}`,
        formData,
        config,
      );
      ...
      if (selectedSection && selectedSection.semesterId) {
        try {
          ...
          const response = await axios.get(
            `${apiUrl}/api/semester/${selectedSection.semesterId._id}`,
            config,
          );
          ...
          toast.error(
            "Failed to load courses and faculties for the selected section",
          );
        }
      }
    }
  ...
  const validateMappingForm = () => {
    ...
    requiredFields.forEach((field) => {
      if (
        ...
        (typeof mappingFormData[field] === "string" &&
          mappingFormData[field].trim() === "")
      ) {
        errors[field] =
          `${field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, " $1")} is required`;
      }
  ...
  const handleAddCourseFacultyMapping = async () => {
    ...
    try {
      ...
      if (response.data.code === 200) {
        ...
        setServerError(
          response.data.message || "Failed to add mapping. Please try again.",
        );
      }
    } catch (err) {
      console.log(err);
      ...
      const errorMessage =
        err.response?.data?.message ||
        "Failed to add mapping. Please check your connection and try again.";
      setServerError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteMapping = async () => {
    ...
    try {
      ...
      if (response.data.code === 200) {
        ...
        setSelectedSectionMappings(
          selectedSectionMappings.filter(
            (mapping) => mapping._id !== currentMapping._id,
          ),
        );
  ...
  return (
    ...
    >
      <Card className="max-w-[1200px] border shadow-lg overflow-hidden">
        <CardHeader className="bg-gradient-to-r from-[#048c7f] to-[#8a1a68] text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                <TabsList className="bg-white/10 border-white/20">
                  <TabsTrigger
                    value="list"
                    className="data-[state=active]:bg-white data-[state=active]:text-[#048c7f] text-white"
                  >
              ...
              >
                ...
                <DialogContent className="sm:max-w-lg bg-white">
                  <DialogHeader className="text-[#048c7f]">
                    <DialogTitle className="font-extrabold text-2xl">
                      Add Course-Faculty Mapping
                    </DialogTitle>
                  </DialogHeader>
                  <DialogDescription>
                    Associate a course with a faculty member for a specific
                    section.
                  </DialogDescription>
                  {serverError && (
                    <motion.div
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      ...
                      <Select
                        onValueChange={(value) =>
                          handleMappingInputChange("sectionId", value)
                        }
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        onValueChange={(value) =>
                          handleMappingInputChange("courseId", value)
                        }
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        onValueChange={(value) =>
                          handleMappingInputChange("facultyId", value)
                        }
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      className="bg-[#048c7f] text-white mt-3 hover:bg-[#5f0a47] hover:text-white transition-all duration-200 shadow-md"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    className="bg-white text-[#048c7f] hover:bg-gray-100 hover:text-[#048c7f] transition-colors duration-300 shadow-md"
                  >
                      ...
                      >
                        ...
                        <TableCell>
                          {section.semesterId ? (
                            <Badge
                              variant="outline"
                              className="bg-[#048c7f]/10 text-[#048c7f] border-[#048c7f]/20"
                            >
                              {section.semesterId.name}
                            </Badge>
                          ) : (
                        ...
                        <TableCell>
                          ...
                          >
                            {section.courseFacultyMappings
                              ? section.courseFacultyMappings.length
                              : 0}
                          </Badge>
                        </TableCell>
                ...
                {getSectionsBySemester().map((group, groupIndex) => (
                  ...
                  >
                    <Card className="border shadow-sm overflow-hidden">
                      <CardHeader
                        className="bg-gradient-to-r from-[#048c7f]/5 to-[#8a1a68]/5 p-4 cursor-pointer"
                        onClick={() =>
                          toggleSemesterExpansion(group.semester._id)
                        }
                      >
                        <div className="flex justify-between items-center">
                          <CardTitle className="text-lg font-medium flex items-center">
                            <BookOpen className="mr-2 h-5 w-5 text-[#048c7f]" />
                            {group.semester.name}
                            <Badge className="ml-3 bg-[#048c7f]/10 text-[#048c7f] hover:bg-[#048c7f]/20 border-[#048c7f]/20">
                          ...
                          <ChevronRight
                            className={`h-5 w-5 text-[#048c7f] transition-transform duration-200 ${
                                      ...
                                      <CardContent className="p-4">
                                        ...
                                        <div className="flex justify-end gap-2 mt-2">
                                          <Button
                                            ...
                                            className="text-blue-500 hover:text-blue-700 hover:bg-blue-50 transition-colors"
                                            onClick={() =>
                                              openViewMappingsDialog(section)
                                            }
                                          >
                                            <svg
                                              xmlns="http://www.w3.org/2000/svg"
        ...
        <CardFooter className="flex justify-between border-t p-4 bg-gray-50/50">
          <div className="text-sm text-gray-500">
            Total: {sections.length} section(s)
          </div>
          <Button
            variant="outline"
      ...
      >
        <DialogContent className="sm:max-w-lg bg-white">
          ...
          <DialogDescription>
            Make changes to the section information below.
          </DialogDescription>
          {serverError && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={submitting}
              className="bg-[#048c7f] text-white hover:bg-[#6c0e51] hover:text-white transition-all duration-200 shadow-md"
            >
              {submitting ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ...
      >
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the section "{currentSection?.name}".
      ...
      <Dialog
        open={isViewMappingsDialogOpen}
        onOpenChange={(open) => setIsViewMappingsDialogOpen(open)}
      >
        <DialogContent className="sm:max-w-lg bg-white">
          <DialogHeader>
                      ...
                      >
                        <TableCell>
                          <div className="font-medium">
                            {mapping.courseId
                              ? mapping.courseId.courseCode
                              : "N/A"}
                          </div>
                          <div className="text-sm text-gray-500">
                            {mapping.courseId ? mapping.courseId.name : ""}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="font-medium">
                            {mapping.facultyId
                              ? `${mapping.facultyId.firstName} ${mapping.facultyId.lastName}`
                              : "N/A"}
                          </div>
                          <div className="text-sm text-gray-500">
                            {mapping.facultyId
        ...
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {currentMapping?.courseId
                ? ` "${currentMapping.courseId.name}"`
                : ""}{" "}
              and
