      ...
      if (filterDepartment !== "all") {
        filteredFaculties = filteredFaculties.filter(
          (faculty) =>
            ...
            faculty.departmentId._id === filterDepartment,
        );
      }

      if (filterDesignation !== "all") {
        filteredFaculties = filteredFaculties.filter(
          (faculty) => faculty.designation === filterDesignation,
        );
      }
    ...
    if (name === "departmentId" && value) {
      ...
      if (department && department.institutionDomain) {
        ...
        if (formData.firstName) {
          const emailPrefix = formData.firstName
            .toLowerCase()
            .replace(/\s+/g, "");
          setFormData((prev) => ({
            ...prev,
  ...
  const handleAddFaculty = async () => {
    ...
    try {
      ...
      if (response.data.code === 201) {
        ...
        setServerError(
          response.data.message || "Failed to add faculty. Please try again.",
        );
      }
    } catch (err) {
    ...
    >
      <Card className="max-w-[1200px] border shadow-lg overflow-hidden">
                    ...
                    <TabsContent value="professional" className="mt-4">
                      <div className="grid gap-4">
                        ...
                        <div className="grid gap-2">
                          ...
                          >
                            <SelectTrigger
                              className={`${formErrors.designation ? "border-red-500 ring-red-200" : ""} transition-all duration-200`}
                            >
        ...
        <CardFooter className="flex justify-between border-t p-4 bg-gray-50/50">
          <div className="text-sm text-gray-500">
            Total: {faculties.length} faculty member(s)
          </div>
          <Button
            variant="outline"
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="mt-1">
                    {currentFaculty.departmentId
                      ? currentFaculty.departmentId.name
                      : "N/A"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="mt-1">
                    {formatDate(currentFaculty.joiningDate)}
                  </p>
                </div>
              </div>
      ...
      >
        <DialogContent className="sm:max-w-lg bg-white">
          ...
          <DialogDescription>
            Make changes to the faculty information below.
          </DialogDescription>
          {serverError && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
        ...
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {currentFaculty?.firstName} {currentFaculty?.lastName}". This
