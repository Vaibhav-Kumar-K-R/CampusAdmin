    ...
    try {
      ...
      if (filterDepartment !== "all") {
        coursesData = coursesData.filter(
          (course) =>
            course.departmentId && course.departmentId._id === filterDepartment,
        );
      }
      ...
      if (searchQuery) {
        ...
        coursesData = coursesData.filter(
          (course) =>
            ...
            (course.departmentId &&
              course.departmentId.name.toLowerCase().includes(query)),
        );
      }
  ...
  const handleAddCourse = async () => {
    ...
    try {
      ...
      if (response.data.code === 201) {
        ...
        setServerError(
          response.data.message || "Failed to add course. Please try again.",
        );
      }
    } catch (err) {
  ...
  const handleUpdateCourse = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/api/course/${currentCourse._id}`,
        formData,
        config,
      );
      ...
      if (response.data.code === 200) {
        ...
        // Handle API success: false response
        setServerError(
          response.data.message || "Failed to update course. Please try again.",
        );
      }
    } catch (err) {
  ...
  const handleUpdateStatus = async (courseId, newStatus) => {
    try {
      ...
      const response = await axios.patch(
        `${apiUrl}/api/course/${courseId}/status`,
        { status: newStatus },
        config,
      );
                              ...
                              <DropdownMenuContent className="bg-white">
                                ...
                                >
                                  <DropdownMenuRadioItem
                                    value="Open"
                                    onClick={() =>
                                      handleUpdateStatus(course._id, "Open")
                                    }
                                    className="cursor-pointer transition-colors"
                                  >
                                    Open
                                  ...
                                  <DropdownMenuRadioItem
                                    value="Closed"
                                    onClick={() =>
                                      handleUpdateStatus(course._id, "Closed")
                                    }
                                    className="cursor-pointer transition-colors"
                                  >
                                    Closed
      ...
      >
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the course "{currentCourse?.name}".
