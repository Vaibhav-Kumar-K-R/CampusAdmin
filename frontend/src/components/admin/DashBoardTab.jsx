  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        let [studentsRes, facultyRes, coursesRes, departmentsRes] =
          await Promise.all([
        ...
        studentsRes.forEach((student) => {
          if (student.departmentId.departmentCode) {
            studentsByDepartment[student.departmentId.departmentCode] =
              (studentsByDepartment[student.departmentId.departmentCode] || 0) +
              1;
          }
        });

        const studentDistribution = Object.keys(studentsByDepartment).map(
          (department) => ({
            department,
        ...
        facultyRes.forEach((faculty) => {
          if (faculty.departmentId.name) {
            facultyByDepartment[faculty.departmentId.name] =
              (facultyByDepartment[faculty.departmentId.name] || 0) + 1;
          }
        });

        const facultyDistribution = Object.keys(facultyByDepartment).map(
          (department) => ({
            department,
