        ...
        semestersData = semestersData.filter(
          (semester) =>
            ...
            (semester.departmentId &&
              semester.departmentId.name.toLowerCase().includes(query)),
        );
      }
  ...
  const handleAddSemester = async () => {
    ...
    try {
      ...
      if (response.data.code === 201) {
        ...
        setServerError(
          response.data.message || "Failed to add semester. Please try again.",
        );
      }
    } catch (err) {
      ...
      const errorMessage =
        ...
        "Failed to add semester. Please check your connection and try again.";
  ...
  const handleUpdateSemester = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/api/semester/${currentSemester._id}`,
        formData,
        config,
      );
      ...
      const errorMessage =
        err.response?.data?.message ||
        "Failed to update semester. Please check your connection and try again.";
      setServerError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setSubmitting(false);
    ...
    >
      <Card className="max-w-[1200px] border shadow-lg overflow-hidden">
                  ...
                  >
                    <Card className="border shadow-sm overflow-hidden">
                      <CardHeader
                        ...
                        onClick={() =>
                          toggleDepartmentExpansion(group.department._id)
                        }
                      >
                        <div className="flex justify-between items-center">
                          <CardTitle className="text-lg font-medium flex items-center">
                            <Calendar className="mr-2 h-5 w-5 text-[#048c7f]" />
        ...
        <CardFooter className="flex justify-between border-t p-4 bg-gray-50/50">
          <div className="text-sm text-gray-500">
            Total: {semesters.length} semester(s)
          </div>
          <Button
            variant="outline"
      ...
      >
        <DialogContent className="sm:max-w-lg bg-white">
          ...
          <DialogDescription>
            Make changes to the semester information below.
          </DialogDescription>
          {serverError && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
      ...
      >
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the semester "{currentSemester?.name}
